...
static poolp usedpools[NB_SMALL_SIZE_CLASSES * 2] = { //这样可以减少不必要的判断
...
static arena_obj* sy_new_arena(/*State *S*/) {
	arena_obj *arenaobj;
...
//	address = skym_alloc(S, NULL, 0, ARENA_SIZE);
	if (address == NULL) {
	...
	sky_assert(POOL_SIZE * arenaobj->nfreepools == ARENA_SIZE);
...
static bool mem_malloc(/*State *S,*/void **ptr, size_t nbytes) {
	uint size;
	poolp pool;
	block *bp;
	poolp next;
	...
	if (pool != pool->nextpool) { //初始情况是相等的，这样不用判断是否为NULL
		++pool->ref.count;
		bp = pool->freeblock;
		assert(bp != NULL);
	...
	if (usable_arenas == NULL) {
		...
		usable_arenas = sy_new_arena(/*S*/); //freepools为NULL，需初始化
		if (usable_arenas == NULL) {
			goto failed;
		}
...
static bool mem_free(/*State *S,*/void *p) {
	poolp pool;
	pool = POOL_ADDR(p);
	block *lastfree;
	poolp next, prev;
...
bool mem_realloc(/*State *S,*/void **ptr, size_t nbytes) {
	void *bp;
	poolp pool;
	size_t size;
	sky_assert(*ptr);
